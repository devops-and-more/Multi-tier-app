...
def removehtmlheaders(htmlcode):
    ...
    if table:
        # Get the content of the table and remove the <table> tags
        table_content = ''.join(str(item) for item in table.contents)
...
def printsite(modulename,formname_or_cmd,formnotes,formcount):
        ...
        if os.path.exists('base.html'):
                ...
                for each in basehtml:
                        ...
                        #This print (s the server information in the HTML title.)
                        ...
                        if each == '<!-- StartAppServerInfo -->':
                                ...
                                try:
                                        x=urllib.request.urlopen('http://{}:8080/appserverinfo.py'.format(AppServerHostname))
                                        data = x.read().decode('utf-8')
                                        appserverhtml = removehtmlheaders(data)
                                        print(appserverhtml)
                                except:
                        ...
                        if each == '<!-- StartCustom -->':
                                ...
                                if modulename != None:
                                        if modulename == 'enterdb':
                                                ...
                                                try:
                                                        encoded_name = urllib.parse.quote(formname_or_cmd)
                                                        encoded_notes = urllib.parse.quote(formnotes)

                                                        # Construct the full URL
                                                        url = 'http://%s:8080/commitdb-app.py?name=%s&notes=%s&count=%s' % (
                                                        AppServerHostname,
                                                        encoded_name,
                                                        encoded_notes,
                                                        formcount
                                                ...
                                                try:
                                                        x=urllib.request.urlopen('http://%s:8080/cleardb-app.py?command=%s'%(AppServerHostname,formname_or_cmd))
                                                        data = x.read().decode('utf-8')
                                                        appserverhtml = removehtmlheaders(data)
                                                        print ( appserverhtml)
                                                except Exception as e:
                                                ...
                                                try:
                                                        x=urllib.request.urlopen('http://%s:8080/%s.py'%(AppServerHostname,modulename))
                                                        data = x.read().decode('utf-8')
                                                        appserverhtml = removehtmlheaders(data)
                                                        print ( appserverhtml)
                                                except Exception as e:
